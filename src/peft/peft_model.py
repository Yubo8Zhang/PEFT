    ...
    **Attributes**:
    ...
    def __init__(self, model, peft_config: PeftConfig):
        ...
        if isinstance(self.peft_config, PromptLearningConfig):
            self._setup_prompt_encoder()   # 我们会在这里冻结base_model的参数并初始化prompt_encoder
        else:
    ...
    def _setup_prompt_encoder(self):
        ...
        # 下面先冻结base_model中的所有模块参数
        for name, module in self.base_model.named_children():
            for param in module.parameters():
                param.requires_grad = False    # 就是这条语句，冻结了模块中的参数
        ...
        # 下面这段代码找到base_model的词嵌入矩阵，并将其存入PeftModel的成员变量
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if inputs_embeds is None:  # 这里用的是base_model的embedding
                inputs_embeds = self.word_embeddings(input_ids)
            prompts = self.get_prompt(batch_size=batch_size)   # 此语句会调用Prompt_encoder，随机生成长度为num_virtual_tokens的soft prompt token
            prompts = prompts.to(inputs_embeds.dtype)
            inputs_embeds = torch.cat((prompts, inputs_embeds), dim=1)  # 把prompt embedding和text embedding拼起来输给模型
